...
window.onload = function () {
  ...
  if (
    localStorage.getItem("autoSave") === "enabled" ||
  ...
  if (localStorage.getItem("fontSize")) {
    ...
    notepad.style.lineHeight =
      parseInt(localStorage.getItem("fontSize")) * 1.5 + "px";
...
function pasteText() {
  navigator.clipboard.readText().then((text) => {
    ...
    notepad.value =
      notepad.value.substring(0, start) + text + notepad.value.substring(end);
...
function printText() {
  ...
  doc.text(
    margin,
    pageHeight - margin,
    "This File was created on online notepad by bkpkvideo.com",
    {
      fontSize: 8,
...
function zoomIn() {
  const currentFontSize = parseFloat(
    window.getComputedStyle(notepad, null).getPropertyValue("font-size")
  );
...
function downloadFile() {
  ...
  switch (fileFormat) {
    ...
    case "word":
      ...
      link.href = `data:application/vnd.openxmlformats-officedocument.wordprocessingml.document;base64,${btoa(
        content
      )}`;
